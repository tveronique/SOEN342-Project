    ...
    const fetchDetailsForBookings = async () => {
      const bookingDetailsPromises = bookings.map(async (booking) => {
        try {      console.log('Booking data:', booking);  // Log the entire booking object
          ...
          const instructorResponse = await axios.get(`/api/users/${booking.instructorPhoneNumber}`);
      ...
      {detailedBookings.map((booking) => (
        <div key={booking.id} className="viewOfferings">
          ...
          <p>
            ...
            <b>{booking.offering?.lesson.private ? "Private lesson" : "Group lesson"}</b>
          </p>
          <p>Instructor: {booking.instructor?.name}</p>
          <p>Instructor Phone: {booking.instructor?.phoneNumber}</p>
        </div>
      ))}
    </div>
  );
